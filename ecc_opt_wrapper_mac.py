...
Parallelized ECC simulation wrapper.
Uses multiprocessing with 'spawn' start method (required for Mac)
to solve (tau, pop) grid points simultaneously across CPU cores.
"""
...
def _solve_one(args):
    ...
    args: (k, j, tech, population, parameters)
...
def run_ecc_simulation(tau=None, pop=None, max_iter=200, n_workers=None):
    ...
    # run in parallel using spawn (required for Mac)
    completed = 0
    with Pool(processes=n_workers) as pool:
        for k, j, result, utiltemp, kmax in pool.imap_unordered(_solve_one, jobs, chunksize=5):
...
if __name__ == "__main__":
    ...
    # small test grid - run this first
    run_ecc_simulation(
